...
import { PackagetypeStateService } from "./services/packagetype-state.service";
...
export class PackagetypeListComponent implements OnInit {
  ...
  constructor(
    ...
    private packagetypeStateService: PackagetypeStateService,
  ...
  ngOnInit() {
    this.breadcrumbItems = [
      ...
      { label: "PackageType", routerLink: "/md/packagetype" },
      { label: "List" },
    ];
    this.restoreStateAndLoadPackageTypes();
  }

  restoreStateAndLoadPackageTypes() {
    this.packagetypeStateService
      ...
      .subscribe((state) => {
        if (state) {
          ...
          this.selectedPackageTypeId = state.selectedPackageTypeId;
  ...
  loadPackageTypes(event?: any) {
    ...
    this.packagetypeService
      .getPackageTypes(page, perPage, this.prepareFiltersForAPI())
      .subscribe(
        (response: PackageType[] | any) => {
        ...
        (error: any) => {
          console.error("Error fetching packagetypes:", error);
          this.loading = false;
        }
      );
  }
  ...
  applyFilters() {
    ...
    console.log("Loading PackageTypes after applying filters.");
    if (this.validateDates()) {
      this.loadPackageTypes();
    }
  }
  resetOneByOneFilters(): void {
    if (this.filters.package_type !== "") {
      this.filters.package_type = "";
      this.applyFilters();
  ...
  createNewRecord() {
    this.router.navigate(["/md/packagetype/create"]);
  }
  ...
  viewRecord() {
    ...
    this.packagetypeStateService.setPackageTypeListState({
      filters: this.filters,
      currentPage: this.currentPage,
      perPage: this.perPage,
      selectedPackageTypeId: this.selectedPackageTypeId,
      navigatedFromListView: this.navigatedFromListView,
    });

    this.router.navigate([
      `/md/packagetype/view/${this.selectedPackageTypeId}`,
    ]);
  }

  editRecord() {
    if (this.selectedPackageTypeId === null) {
      this.showError("Please select a packagetype to edit.");
      return;
    }

    this.router.navigate([
      `/md/packagetype/edit/${this.selectedPackageTypeId}`,
    ]);
  }

  deactivateRecord() {
    if (this.selectedPackageTypeId === null) {
      this.showError("Please select a packagetype to deactivate.");
      return;
    }

    this.confirmationService.confirm({
      message: "Are you sure you want to deactivate this packagetype?",
      accept: () => {
        if (typeof this.selectedPackageTypeId === "number") {
          this.packagetypeService
            .deactivatePackageType(this.selectedPackageTypeId)
            .subscribe(
              () => {
                this.showSuccess(
                  `PackageType with ID ${this.selectedPackageTypeId} deactivated successfully`
                );
                this.loadPackageTypes();
              },
              (error: any) => this.showError("Error deactivating packagetype")
            );
        }
      },
    });
  ...
  deleteRecord() {
    if (this.selectedPackageTypeId === null) {
      this.showError("Please select a packagetype to delete.");
      return;
    }

    this.confirmationService.confirm({
      message: "Are you sure you want to delete this packagetype?",
      accept: () => {
        if (typeof this.selectedPackageTypeId === "number") {
          this.packagetypeService
            .deletePackageType(this.selectedPackageTypeId)
            .subscribe(
              () => {
                this.showSuccess(
                  `PackageType with ID ${this.selectedPackageTypeId} deleted successfully`
                );
                this.loadPackageTypes();
              },
              (error: any) => this.showError("Error deleting packagetype")
            );
        }
      },
    });
  ...
  onRowSelect(event: any) {
    this.selectedPackageTypeId = event.data.id;
  }
